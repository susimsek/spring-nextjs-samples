...
const Home = () => {
  ...
  const renderContent = () => {
    switch (activePage) {
      ...
      case 'api':
        return <EmbeddedContentFrame src={`${apiBaseUrl}/swagger-ui.html`} title={t('common:common.menu.api')} />;
      case 'gqlApi':
        return <EmbeddedContentFrame src={`${apiBaseUrl}/graphiql`} title={t('common:common.menu.gqlApi')} />;
      case 'database':
        return <EmbeddedContentFrame src={`${apiBaseUrl}/h2-console`} title={t('common:common.menu.database')} backgroundColor="#ffffff" />;
